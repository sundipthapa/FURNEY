...
const Navbar = () => {
    ...
    const postproduct = useNavigate();

    const handlePostClick = () => {
        postproduct.navigate('/post');
    };
    ...
    return (
        <nav className="bg-amber-900">
            ...
            <div className="w-full h-24 flex justify-between items-center border-b border-gray-200 px-4">
                <div className="text-white text-4xl font-bold">FURNEY</div>
                <div className="flex items-center space-x-4">
                    ...
                    <a href="/add-to-cart" className="text-2xl text-white">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
            ...
            <div className="hidden md:flex w-full h-16 justify-between items-center bg-amber-900 px-4">
                <ul className="flex space-x-4 font-semibold text-white justify-center items-center">
                    {navItems.map((item, index) => (
                        <li key={index} className="cursor-pointer px-12 py-4 hover:bg-white hover:text-black h-16 text-xl">{item}</li>
                    ))}
                </ul>
                <button className="bg-amber-700 text-white font-semibold px-4 py-2 rounded">
                    Post
                </button>
            </div>
            ...
            {navOpen && (
                <div className="md:hidden">
                    <ul className="bg-amber-900 w-full px-4 space-y-2 py-4 text-white">
                        {navItems.map((item, index) => (
                            <li key={index} className="cursor-pointer py-2 border-b border-gray-600 hover:bg-white hover:text-black text-xl">{item}</li>
                        ))}
